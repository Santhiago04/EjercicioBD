...
export const get_categorias_con_a = async (_, respuesta) => {
  try {
    // Usamos una expresión regular para buscar categorías que comiencen con "A"
    let categorias = await categoriaModel.find({ nombre: { $regex: /^A/, $options: 'i' } });

    // Mostrar las categorías filtradas en la consola
    console.log('Categorías que comienzan con "A":', categorias);

    // Enviamos las categorías filtradas como respuesta
    respuesta.status(200).json(categorias);
  } catch (error) {
    console.error('Error al obtener las categorías que comienzan con "A":', error);
    respuesta.status(500).json({ message: "Error al obtener las categorías", error });
  }
};
